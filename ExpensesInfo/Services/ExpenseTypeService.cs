...
namespace ExpensesInfo.Services
{
    public class ExpenseTypeService : IExpenseTypeService
    {
        ...
        public ExpenseTypeService(ExpensesInfoDbContext context) => _context = context;
        ...
        {
            _context.ExpenseTypes.Add(type); await _context.SaveChangesAsync();
        }

        public async Task UpdateAsync(ExpenseType type)
        {
            var existing = await _context.ExpenseTypes.SingleOrDefaultAsync(t
=> t.Id == type.Id); if (existing == null) return;

            existing.Name = type.Name; await _context.SaveChangesAsync();
        }

        public async Task DeleteAsync(int id)
        {
            var existing = await _context.ExpenseTypes.SingleOrDefaultAsync(t
=> t.Id == id); if (existing == null) return;

            _context.ExpenseTypes.Remove(existing); await _context.SaveChangesAsync();
        }
    }
}
